...
function addEventListenersNotAuth() {
  ...
  document.getElementById('loginUserButton').addEventListener('click', function () {
    ...
    const url = `http://localhost:4242/user/check_user_credentials/${usernameElement.value}/${passwordElement.value}/`
    fetch(url)
      .then(response => {
        if (!response.ok) {
  ...
  document.getElementById('RegisterUserButton').addEventListener('click', function () {
    ...
    const url = `http://localhost:4242/user/account/create/${usernameElement.value}/${passwordElement.value}/${age}/`
    fetch(url)
      .then(response => {
        if (!response.ok) {
          switch (response.status) {
