...
//     try {
//       const response = await fetch(`http://localhost/user/game/create/${websocket_obj.username}/${websocket_obj.invited_id}`);
...
// }
  ...
  document.getElementById('challengeUserToGame').addEventListener('click', async function() {
    ...
    try {
      const response = await fetch(`https://${window.location.hostname}/user/game/create/${websocket_obj.username}/${websocket_obj.invited_id}`);
      const data = await response.json();
